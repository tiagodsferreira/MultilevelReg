---
title: "Analises Multinível - Laboratório (Parte 3)"
...
Laboratório (Parte 2) | Breve introdução ao R:   
...
O seguinte código serve apenas para demonstrar como foi criada a base de dados que será utilizada ao longo deste tutorial. Se desejar apenas importar a base de dados poderá simplesmente correr o código para importação de ficheiro csv que está disponível logo no início do ponto 1, *Importar e Explorar a base de dados*.
...
write.csv(All.Schools.Data, "I:\\My Drive\\FPCEUP\\AtividadeDocente\\MultilevelReg\\MLReg_data\\SchoolsData.csv", row.names=FALSE)
...
Em vez de criar a base de dados do zero com o código anterior, poderá simplesmente importá-la do ficheiro csv disponibilizado no materiais de curso utilizando o código seguinte.   
```{r}
library(data.table)  # se necessário: install.packages("data.table")
SchoolsData <- fread("I:\\My Drive\\FPCEUP\\AtividadeDocente\\MultilevelReg\\MLReg_data\\SchoolsData.csv")
```

A base de dados "SchoolsData" é composta por 600 alunos inseridos em 6 escolas, 3 em contextos economicamente favorecidos e 3 em contextos economicamente desfavorecidos. A variável em estudo é o nível de felicidade ("Happiness") reportado por cada um dos alunos. Pretendemos explicar "Happiness" (variável dependente) em função do número de amigos ("Friends"), qualidade da escola percecionada por cada um dos alunos ("Quality") e aproveito académico ("GPA"). Começamos por usar algumas funções já conhecidas para perceber a estrutura geral da base de dados.
...
Exploramos agora graficamente a base de dados. As escolas 1, 2 e 3 estão situadas em contextos economicamente mais favorecidos, enquanto as escolas 4, 5 e 6 em contexto menos favorecidos. Usamos para o efeito o pacote ggplot2.
...
Começamos por criar um gráfico para verificar a relação entre os scores de felicidade do alunos ("Happiness" - VD) e número de amigos ("Friends", VI).  
...
Pelo gráfico anterior percebemos que, embora pareça existe um efeito positivo entre as duas variáveis, no entanto, esses efeitos são mais evidentes nas escolas inseridas em contextos desfavorecidos (escolas nr. 4, 5 e 6).  
...
Aqui, a relação (slope) entre as duas variáveis é semelhante nos dois tipos de escolas, embora pareçam existir diferenças nos intercepts dessas slopes (i.e., níveis médios de felicidade quando o valor de GPA é 0).  
Analisamos agora as correlações entre as diferentes variáveis para a amostra completa, incluindo crianças em escolas de contexto favorecidos e desfavorecidos.
```{r}
names(SchoolsData)
...
Olhemos para as mesmas correlações, separadamente para os dois tipos de escolas na amostra.  

- Para escolas em contextos favorecidos...  
```{r}
...
- Para escolas em contextos desfavorecidos...  
...
É de notar que o padrão de correlações parece mudar entre os dois grupos de escolas, sendo diferente do padrão de correlações observado para a amostra total. Estes resultados sugerem a presença de relações diferentes entre as variáveis observadas em função do tipo de escola considerado.


# 2. Regressão simples  
De seguida, testamos um modelo de regressão simples. Esta abordagem analítica não considera o facto dos dados estarem organizados de forma hierárquica, isto é não tem em conta o facto dos estudantes na amostra partilharem as mesmas escolas. Para facilitar a interpretação dos dados, vamos começar por centrar as variáveis à “grand mean”. Para isso, subtraímos  a média de uma determinada variável ao valores individuais observados para cada um dos sujeitos nessa mesma variável. Conseguimos facilmente fazer isto através da função "scale()" com o argumento scale = FALSE.  
```{r}
# ?scale
...
yij = *β*0 + *U*0j + *ε*ij  
**Fixed effects:**  
*β*0 - intercept do modelo  
**Random effects:**  
*U*0j - variância do grupo para o intercept  
*ε*ij - variância individual  
...
```  
Com estes dados, podemos agora calcular facilmente o ICC = var. group / var. total, sendo que a var. total = var. grupo + var. indivíduos. 
```{r}
vc[1,2]
vc[,2]
ICC <- vc[1,2]/sum(vc[,2])
ICC
```  
Assim, verificamos que o cluster explica cerca de 20% da variância de “happiness”, o que justifica uma abordagem multi-nível.  
...
```  
Assim, apenas temos que especificar o nome do modelo nulo como argumento e a função ICC_user faz o resto.
```{r}
ICC_user(Null)
```  
...
yij = *β*0 + *β*1(*GPA*)ij + *U*0j + *ε*ij  
**Fixed effects:**    
*β*0 - intercept do modelo  
*β*1(*GPA*)ij - Efeito médio de GPA sobre happiness  
**Random effects:**   
*U*0j - variância do grupo  
*ε*ij - variáncia individual   
...
```
O output porvidencia 2 indicadores. O R2m consiste na variância explicada apenas pelos fixed effects, enquanto que o indicador R2c representa a variância explicada pela totalidade do modelo, considerando tanto os fixed como os random effects.
...
No modelo que se segue incluímos a interação entre preditores de nível 1 (i.e., indivíduo). Esta interação é definida na sintaxe do modelo, incluindo as duas variáveis separadas pela operador "*".  
```{r}
...
A função ranova() pode-nos ajudar a melhorar o modelo, uma vez que permite verificar o efeito da retirada de cada um dos random effects do modelo  
